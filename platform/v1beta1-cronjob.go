...
func execV1Beta1CronJobSetResouce(k kubernetes.Interface, sts *v1beta1.CronJob, namespace string, opts configuration.Options, commandFlag configuration.CommandFlag) (state.State, error) {
	...
	if commandFlag == configuration.Apply {
		...
		if err != nil {
			log.Error(fmt.Sprintf("Could not apply CronJob resource %s due to %s", sts.Name, err.Error()))
